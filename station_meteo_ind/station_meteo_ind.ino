...
void loop() {
    ...
    if (rstReason == "External System" && firstRestart < 0) {
        ...
        display.powerOff();
        if (timeElapsed > 0) ESP.deepSleep(timeElapsed * 1000000, WAKE_RFCAL);
        yield();
    }
#else
...
void readTemperature() {
    ...
    http.begin(client, String(THINGSPEAK_POST) + "&field1=" + tempIntCurr + "&field3=" + humIntCurr);
...
void displayWeatherItem(int weath, int posX, int posY, int width, int height) {
    if (weath == STORM)
        display.drawInvertedBitmap(posX, posY, image_data_storm, width, height, GxEPD_BLACK);
        ...
        display.drawInvertedBitmap(posX, posY, image_data_fog, width, height, GxEPD_BLACK);
    else if (weath == SUNNY_CLOUD)
        display.drawInvertedBitmap(posX, posY, image_data_sunny_cloud, width, height, GxEPD_BLACK);
    else if (weath == CLOUD)
        display.drawInvertedBitmap(posX, posY, image_data_cloud, width, height, GxEPD_BLACK);
    else if (weath == SUN)
    ...
    if (httpCode == HTTP_CODE_OK) {
        ...
        *value = -99;
        if (lastI != http.getString().length()) {
            String tmp = http.getString().substring(lastI + 1, http.getString().length());
...
void getSunWeather() {
    ...
    if (httpCode == HTTP_CODE_OK) {
        ...
        if (code >= 200 && code <= 232) weather = 0;

        if (code >= 300 && code <= 321) weather = 1;

        if (code >= 500 && code <= 531) weather = 2;

        if (code >= 600 && code <= 622) weather = 3;

        if (code >= 700 && code <= 781) weather = 4;

        if (code >= 801 && code <= 804) weather = 5;

        if (code == 800) weather = 6;
...
#pragma endregion
    ...
    Serial.printf("Sun : %d:%d - %d:%d\n", sunsetHour, sunsetMin, sunriseHour, sunriseMin);
